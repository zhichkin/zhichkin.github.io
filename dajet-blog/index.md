---
title: Блог DaJet
description: Статьи и прочие полезные материалы
---
[Главная](../index.md#блог-dajet)

### Логическая репликация 1С:Предприятие 8
- [Формат JDTO (JSON data transfer object)](/1c-logical-replication/jdto)
- [Подсистема "Механизм репликации" для 1С](/1c-logical-replication)

### Презентации (pdf)
1. [Методика РИБ 2.0](/pdf/distributed-info-bases-design.pdf)
2. [Эволюция схем данных](/pdf/data-schema-evolution.pdf)
3. [Особенности интеграции OLTP-систем (transactional outbox pattern)](/pdf/OLTP-transactional-outbox-pattern.pdf)

### Видео-уроки (mp4)
1. [Тюнинг планов обмена](/mp4/exchange-plan-tuning.mp4)
2. [Захват изменения данных регистров сведений](/mp4/info-register-change-data-capture.mp4)